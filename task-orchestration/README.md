...
- **Machine 2** owns `machine-2-parallel-tracks/` — six independent feature tracks that do
...
1. **Anytime, any order, fully parallel:** all of `machine-2-parallel-tracks/*` may be
   ...
   this document. Each of the six tracks (`01`-`06`) is its own branch/PR; they touch disjoint
...
2. **Must merge before any `post-tenancy-*` branch is created:** `machine-1-tenancy-core`,
   in its internal chunk order `01 → 02 → 03 → 04` (each chunk's migration/model depends on the
   previous chunk's schema). This is one branch, `feat/tenancy-core`, reviewed as up to four
   stacked PRs or one PR — implementer's choice — but it must be fully merged before step 3.
...
4. **Only after `post-tenancy-retrofit/04`'s test suite is green on real dockerized Postgres in
   CI:** `post-tenancy-features/01` and `02` may branch and merge, in either order.

## Subagent role assignment

| Track | Developer | Reviewer | Tester | Notes |
|---|---|---|---|---|
| `machine-1-tenancy-core` (all 4 chunks) | 1 developer subagent, sequential dispatch (chunk N waits for chunk N-1's migration to exist) | 1 reviewer subagent per chunk, gates progression to next chunk | 1 tester subagent after chunk `04`, full auth+CORS+rate-limit test pass | Single-threaded by design — this is the track everything else waits on, so speed here matters more than parallelism |
| `machine-2-parallel-tracks/01, 02` | 1 developer subagent per track, dispatched in parallel | 1 reviewer subagent per track | 1 tester subagent per track | Independent CV/profiling and JobSpy-country domains, zero file overlap |
...
| `machine-2-parallel-tracks/04` | 1 developer subagent | 1 reviewer subagent | 1 tester subagent | Independent of the 03→05→06 chain |
...
- `feat/tenancy-core` — machine-1, all four chunks
- `feat/progressive-profiling-fields`, `feat/country-demand-intelligence`,
  ...
  `feat/outreach-canspam-compliance`, `feat/linkedin-outreach-send` — machine-2, one branch per
  track (the `03 → 05 → 06` sub-chain may be three stacked branches or three commits on one
  branch, implementer's choice, as long as each is reviewable independently)
...
- `feat/billing-stripe`, `feat/brand-landing-pages` — post-tenancy-features
