...
Six independent feature tracks that extend the product **without** touching tenant/`org_id`
...
schema `03` defines and the suppression primitives `05` defines.
...
chunks) since each later chunk imports the previous chunk's schema/service additions.

## Why these are parallel-safe with `machine-1`

None of these six tracks add `org_id` filtering to any query, create the `Organization` model,
