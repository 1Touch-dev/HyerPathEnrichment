...
async def check_service(name: str, url: str, expected_status: int = 200) -> Dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            status = "✅ UP" if response.status_code == expected_status else f"⚠️  HTTP {response.status_code}"
            return {"service": name, "url": url, "status": status, "healthy": response.status_code == expected_status}
        ...
        return {"service": name, "url": url, "status": f"❌ ERROR: {type(e).__name__}", "healthy": False}
...
async def check_reacher(url: str) -> Dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            status = "✅ UP" if response.status_code in [200, 400] else f"⚠️  HTTP {response.status_code}"
            return {"service": "Reacher (SMTP)", "url": url, "status": status, "healthy": True}
    except Exception as e:
        return {"service": "Reacher (SMTP)", "url": url, "status": f"❌ ERROR: {type(e).__name__}", "healthy": False}
...
async def main():
    ...
    core_checks = [
        ...
        check_service("Redis", "http://127.0.0.1:6379", expected_status=400),  # Will fail but shows it's listening
