...
down_revision: Union[str, Sequence[str], None] = '006_add_user_authentication'
...
def upgrade() -> None:
    ...
    if is_sqlite:
        with op.batch_alter_table('dsar_requests', schema=None) as batch_op:
            ...
            batch_op.create_foreign_key('fk_dsar_requests_user_id', 'users', ['user_id'], ['id'], ondelete='CASCADE')
            batch_op.create_index('ix_dsar_requests_user_id', ['user_id'])
    else:
        op.add_column('dsar_requests', sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=True))
        op.create_foreign_key('fk_dsar_requests_user_id', 'dsar_requests', 'users', ['user_id'], ['id'], ondelete='CASCADE')
