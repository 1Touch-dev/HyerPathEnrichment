...
down_revision: str | Sequence[str] | None = "005_add_is_internal_flag"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "used", sa.Boolean(), nullable=False, server_default="0" if is_sqlite else "false"
        ),
    ...
    op.create_index(
        "ix_logged_out_tokens_token_jti", "logged_out_tokens", ["token_jti"], unique=True
    )
    ...
    if is_sqlite:
        with op.batch_alter_table("jobs", schema=None) as batch_op:
            ...
            batch_op.create_foreign_key(
                "fk_jobs_user_id", "users", ["user_id"], ["id"], ondelete="SET NULL"
            )
