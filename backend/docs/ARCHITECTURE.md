...
| Voice-tone analysis produces a hire/no-hire or "confidence" signal | It does not, by design — `voice_tone_signals` (nullable, `HUME_API_KEY`-gated, off by default) is surfaced only as coaching-framed descriptive text, never a numeric score, never fed into `ai_score` or any ranking (ADR 0015 Decision 2). |
