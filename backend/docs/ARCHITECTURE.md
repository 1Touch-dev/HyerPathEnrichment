...
| Impersonation JWT claims are sufficient authorization | They are not. The frozen contract is candidate-only and `view_only`; every impersonated request must validate the session/JTI, expiry/revocation, real actor, and current permission. Schema support exists, but endpoint/dependency enforcement is a later hardening task. |
...
| Admin Module (RBAC, audit, flags, impersonation) | `app/modules/admin/`, 4 new `users` columns | Real, scaffolded per `phase2_admin_module.md` (ADR 0015). `is_superuser` remains authoritative and RBAC is database-resolved per request. Revisions 063–066 add contract storage for request-correlated audit outcomes, candidate-only view-only impersonation revocation, secure staff-invite transition fields, and privileged idempotency. Endpoint/service enforcement is intentionally not yet implemented. Feature-flag mutations remain disabled pending a consumer; P4/four-eyes operations are unavailable (ADR 0021). |
